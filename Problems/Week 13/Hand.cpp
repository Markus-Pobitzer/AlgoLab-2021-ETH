...
// tc = TentCount with 1, 2, 3, or >= 4 tents
int getFamilies(const int k, const std::vector<int>& tc) {
    int ret = 0;
    switch (k) {
    case 1: {
        ret = tc[0] + tc[1] + tc[2] + tc[3];
        break;
    }
    case 2: {
        ret = tc[0] / 2 + tc[1] + tc[2] + tc[3];
        break;
    }
    case 3: {
        int p12 = std::min(tc[0], tc[1]);
        ret = p12 + (tc[0] - p12) / 3 + (tc[1] - p12) / 2 + tc[2] + tc[3];
        break;
    }
    case 4: {
        int p13 = std::min(tc[0], tc[2]);
        int p2 = tc[1] / 2;
        int p1 = (tc[0] - p13) / 4;
        int p3 = (tc[2] - p13) / 2;
        ret = p1 + p2 + p3 + p13 + ((tc[0] - p13 - 4 * p1) + 2 * (tc[1] % 2) + 3 * (tc[2] - p13 - 2 * p3)) / 4 + tc[3];
    }
    }
    return ret;
}
...
void testCase() {
    ...
    for (EdgeV::const_iterator e = edges.begin(); b2 && e != edges.end(); ++e) {
        // determine components of endpoints
        Index c1 = uf.find_set(std::get<0>(*e));
        Index c2 = uf.find_set(std::get<1>(*e));
        //std::cout << getFamilies(k, tc) << "\n";
        if (c1 != c2) {
            if (getFamilies(k, tc) < f0) {
                b2 = false;
            }
            else {
                s = std::get<2>(*e);
            ...
            uf.link(c1, c2);
            tc[std::min(4, count[c1]) - 1] -= 1;
            tc[std::min(4, count[c2]) - 1] -= 1;
            count[c1] += count[c2];
            count[c2] = count[c1];
            tc[std::min(4, count[c1]) - 1] += 1;
        }
    ...
    bool b = true;
    // ... and process edges in order of increasing length
    for (EdgeV::const_iterator e = edges.begin(); e != edges.end() && b; ++e) {
        // determine components of endpoints
        ...
        if (c1 != c2) {
            if (std::get<2>(*e) < s0) {
                // this edge connects two different components => part of the emst
                uf2.link(c1, c2);
                tc[std::min(4, count2[c1]) - 1] -= 1;
                tc[std::min(4, count2[c2]) - 1] -= 1;
                count2[c1] += count2[c2];
                count2[c2] = count2[c1];
                tc[std::min(4, count2[c1]) - 1] += 1;
            }
